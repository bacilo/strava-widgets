...
- [x] **Phase 20: Row-Click Interaction Pattern** - Every activity row, on every screen, is clickable, keyboard-accessible, and free of redundant "View Activity" buttons (5 plans executed 2026-08-13; NOT complete — `20-VERIFICATION.md` scored 2/4 must-haves, `status: gaps_found`. Two BLOCKER gaps, both confirmed against source: CR-01, `highlightAndFocus` at `list.ts:963` does `querySelector('a')` on a card element that this phase made the anchor itself, so return-from-detail focus restoration is dead below 720px; CR-02, `appendStatusBadges` writes badges into an `<a>` whose curated `aria-label` overrides them, dropping status badges from the accessible name — extends to `overview.ts:108-112` as well, which `20-REVIEW.md` did not catch. UX-01 and UX-03 not satisfied; REC-08 and UX-02 verified. Criterion 4 not defensibly discharged: `20-VALIDATION.md` is `status: partial`, `nyquist_compliant: false` — a blanket approval over 11 of 12 rows with no stated theme coverage for the 7 theme-sensitive rows, the same evidence shape `19-VERIFICATION.md` rejected for UI-02. Gap closure required) (completed 2026-08-13)
