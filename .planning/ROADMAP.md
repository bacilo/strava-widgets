...
- [ ] **Phase 23: Trends Zoom, Pan & Taller Bands** - User can zoom and pan trend charts with mouse, touch, or on-screen controls, on taller bands, without breaking tab-cycling or the canvas lifecycle (7/7 plans executed 2026-08-19; NOT complete — **REOPENED 2026-08-26 by gap closure.** Plan 23-07's Round 1 browser checkpoint closed all 20 rows on individually-quoted evidence at **16 PASS / 4 FAIL / 0 BLOCKED**, and `23-VALIDATION.md` is `status: partial` / `nyquist_compliant: false`. TRN-01 is the first requirement in this phase to have every mapped row PASS and is ticked; TRN-02, TRN-03 and TRN-04 stay Pending on four located, reproducible defects — not on missing evidence. **Finding 10 (root cause, blocks TRN-02 via R11 and TRN-04 via R16):** `buildZoomOptions()` in `chart-zoom.ts:167-168` returns `onZoomComplete`/`onPanComplete` as top-level siblings of the `zoom`/`pan` option objects, but chartjs-plugin-zoom reads them from inside those objects (`state.options.zoom.onZoomComplete` at plugin lines 388/616/674/767, `state.options.pan.onPanComplete` at line 800), so `settle()` never runs on any gesture — a real Command+wheel moved rendered ticks to `11 Aug 2011`…`13 Aug 2026` while the `aria-label` stayed `Weekly distance chart, Aug 2025 to Aug 2026` and Reset never appeared. Subsumes Finding 4. **Finding 1 (blocks TRN-02 via R8):** `chart.zoom(ZOOM_FACTOR)` with `ZOOM_FACTOR = 1.5` halves the span rather than dividing it by 1.5, because the plugin's `linearZoomDelta` computes `newRange = range * (zoom - 1)` and removes that much — observed 6 months against the designed 8. **Finding 9 (blocks TRN-03 via R15):** `.year-heatmap` is a fixed 634px grid (53 tracks x `min-width: 10px` + 52 x 2px gap) that never reflows, pinning `document.documentElement.scrollWidth` at 682 against clientWidth 390/393/412/430, so only R15's non-waivable no-horizontal-overflow clause fails — the D-21 band floor itself holds exactly at all four widths. Findings 7 and 8 are recorded and gate nothing; Findings 5 and 6 are explicitly out of scope and must not gate any TRN requirement. **Gap-closure round planned 2026-08-26 as plans 23-08..23-11 in waves 6-7**, verified by gsd-plan-checker with every load-bearing claim independently re-derived against live source and `node_modules`: 23-08 nests both callbacks correctly and reroutes the four buttons through new pure `zoomStepRange`/`panStepRange` math; 23-09 contains the heatmap in a `.splits-scroll`-style `.year-heatmap-scroll` wrapper; 23-10 adds step-aware tick formatting in a new pure module plus a per-chart `ResizeObserver`; 23-11 runs a BLOCKING 22-row Round 2 checkpoint (R21-R42) re-asking all 20 Round 1 rows plus 2 new ones, carrying forward 23-07's non-waivable house rule 14, and re-gates TRN-01..04 strictly on the Round 2 row map.)
