...
- [ ] **Phase 23: Trends Zoom, Pan & Taller Bands** - User can zoom and pan trend charts with mouse, touch, or on-screen controls, on taller bands, without breaking tab-cycling or the canvas lifecycle (11/11 plans executed 2026-08-27; NOT complete — gate still open on one requirement. **Round 2 gap-closure checkpoint closed 2026-08-26/27 (plan 23-11)** against a fresh build (`assets/index-D01ardNQ.js`, differing from Round 1's `assets/index-D2l-GZfl.js`): **21 PASS / 1 FAIL (R35) / 0 BLOCKED** across R21-R42. Plan 23-08's settle-nesting fix and ÷1.5/×1.5 step-magnitude fix, and plan 23-09's year-heatmap scroll wrapper, together closed all three of Round 1's other FAILs (R8, R11, R16) — **TRN-01, TRN-02 and TRN-04 all tick complete.** **TRN-03 stays Pending**, blocked by R35: 23-09's wrapper narrowed the phone-width horizontal overflow from Round 1's pinned `documentElement.scrollWidth` 682 down to 460, but 460 still exceeds `clientWidth` (390/393/412/430) at every required phone width — the row's own non-waivable no-horizontal-overflow clause needs equality, not improvement. **Finding 11 (new root cause, blocks TRN-03 via R35):** the overflow now traces to a *different* element than Round 1's Finding 9 — the five-tab Trends navigation strip (`div.segmented`, Volume/YoY/Cadence & HR/Training Load/Gear), measured 412px wide with `overflow-x: visible` on itself and its parent, never given a scroll wrapper (unlike the 3-button granularity `.segmented`, which is fine). Only manifests below ~460px viewport width. **Finding 12 (new, gates nothing):** the Training Load tooltip title still renders a raw epoch-millisecond value instead of a formatted date — a re-observation of Round 1's out-of-scope Finding 6 against the fresh build, confirming it survived 23-08/09/10 unchanged; plausibly related to 23-10's tick-formatter change is an unverified hypothesis. Both findings recorded verbatim and left unpatched per house rule 4. `23-VALIDATION.md` is `status: partial` / `nyquist_compliant: false`. Next: a narrow gap-closure plan targeting Finding 11 — wrap or constrain the five-tab `div.segmented` strip — would close TRN-03 and the phase gate.)
