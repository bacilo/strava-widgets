...
> Archived from `.planning/ROADMAP.md` at the v2.1 milestone close. This file preserves the full
> phase details, wave breakdowns and per-plan checklists; the live ROADMAP.md keeps only the
> collapsed one-line-per-phase summary. For the shipped-milestone narrative and stats see
> `.planning/MILESTONES.md`; for requirement outcomes see `v2.1-REQUIREMENTS.md`.
>
> **Note on Phase 22:** its `22-VERIFICATION.md` was stale when this milestone closed — dated
> 2026-08-19T09:30:00Z at `gaps_found` 5/8, it was the report that triggered the phase's own Round 4
> gap-closure work and was never re-run afterward. Re-verified 2026-09-05 to `passed` 8/8, with each
> closure re-derived from source and mutation-tested. Two checkpoint rows (R26's ~600px band, R27's
> three-click aria-label sequence) still stand only on developer authority; both shortfalls are
> retained verbatim in `22-VALIDATION.md` rather than laundered, and neither is load-bearing for a
> verified truth.
