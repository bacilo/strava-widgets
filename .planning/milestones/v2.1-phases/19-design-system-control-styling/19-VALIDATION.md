...
requires its own independent PASS/FAIL verdict token in its own Observation cell; a
...
| 14. The Trends tab list — 5 options, 3 middles (Year-over-Year, Cadence & HR, Training Load) — renders every middle option square-jointed, never a rounded pill, active or inactive | UI-02 | Computed corner geometry on a real rendered box; no CSSOM in this test run. CR-02 (`19-REVIEW.md`). | On `#/trends`, look at the tab bar — Volume, Year-over-Year, Cadence & HR, Training Load, Gear. The three middle tabs must have square joints on both inner sides; only the outer corners of Volume and Gear may be rounded. Click through each middle tab so each is seen filled with `--accent-strong` — an active middle rendering as a fully-rounded pill is the exact defect. Both themes. | **Round 3 2026-08-13:** Trends tab list (5 options, 3 middles — Year-over-Year, Cadence & HR, Training Load) checked against Probe A's second, full run (see Round 3 Probe Outputs, Probe A), executed on the Training Load tab after the default-view run returned only two incomplete groups: Volume 4px 0px 0px 4px, Year-over-Year 0px, Cadence & HR 0px, Training Load 0px, Gear 0px 4px 4px 0px — every middle option computes exactly 0px on all sides and the two end options keep their outer rounding; each middle tab was clicked through and rendered square rather than as a rounded pill in its active fill. The developer was asked explicitly to flip between light and dark theme and re-glance at this row, answering 'looks fine' — a terse both-theme confirmation given in direct response to that explicit prompt, recorded as given rather than expanded into invented per-theme detail. VERDICT: PASS. |
| 15. The Volume granularity group — Weekly / Monthly / Yearly, 1 middle (Monthly) — renders Monthly square on both sides, active and inactive | UI-02 | Same mechanism as row 14, a distinct rendered group. CR-02. | On the `#/trends` Volume tab, the Weekly / Monthly / Yearly group. "Monthly" must be square on both sides, both while active (filled) and while inactive. Both themes. | **Round 3 2026-08-13:** Volume granularity group (Weekly / Monthly / Yearly) checked against Probe A's Volume-tab run (see Round 3 Probe Outputs, Probe A): Weekly 4px 0px 0px 4px, Monthly 0px, Yearly 0px 4px 4px 0px — Monthly computes exactly 0px on both sides, both selected and unselected, matching the row's expectation with no rounded notch on either edge. The developer's both-theme confirmation is the same terse 'looks fine' given once in response to the explicit light/dark prompt spanning rows 14 through 17; recorded here as given, not expanded into separate per-theme sentences that were never spoken. VERDICT: PASS. |
| 16. The Training-load window group — 3mo / 12mo / All, 1 middle (12mo) — renders 12mo square on both sides, active and inactive, and the adjacent two-option Edwards/Banister group remains the unaffected reference | UI-02 | Same mechanism as rows 14-15, plus a direct no-regression comparison. CR-02. | On the `#/trends` Training Load tab, the 3mo / 12mo / All group. "12mo" must be square on both sides, active and inactive. Both themes. While on this tab, also look at the two-option Edwards/Banister model group beside it — say whether the two groups read as the same component (that is the direct comparison for whether the end caps still round correctly). | **Round 3 2026-08-13:** Training-load window group (3mo / 12mo / All) checked against Probe A's Training-Load-tab run (see Round 3 Probe Outputs, Probe A): 3mo 4px 0px 0px 4px, 12mo 0px, All 0px 4px 4px 0px — 12mo computes exactly 0px on both sides. The adjacent two-option TRIMP model group (Edwards/Banister) computed Edwards 4px 0px 0px 4px, Banister 0px 4px 4px 0px — the correct two-option end-cap shape, with no middle option to test, confirming it as the unaffected reference. Developer's verbatim words: 'yes. They are very close to each other so at first sight they might seem like the same. But quickly you see that two boxes are selected and with the rounded corners at the edges it becomes visible. Some more separation wouldn't hurt though.' The 'some more separation' remark is spacing feedback between the training-load-window group and the TRIMP-model group, logged as a UX backlog note and explicitly not counted against this row. Both-theme confirmation: the same 'looks fine' answer given once to the explicit light/dark prompt spanning rows 14-17. VERDICT: PASS. |
| 17. A focused calendar rest day shows a clearly visible focus ring against the dimmed button, while still reading as non-interactive (muted label, no hover highlight) | UI-02 | Perceptual ring legibility against a dimmed control; the contrast figures are closed-form per D-11, but whether the ring reads is not, and whether the D-07 disabled affordance survived being un-dimmed while focused is a separate question. CR-03. | On `#/calendar`, Tab into the day grid and land on a rest day — a day showing "–" rather than a distance. The focus ring must be clearly visible against the dimmed button. Confirm separately that the day still reads as non-interactive while focused (muted label, no hover highlight). Both themes; check light first, it is the narrower margin (3.40:1 vs dark's 6.02:1). | **Round 3 2026-08-13:** A focused calendar rest day was tested via Probe C (fully verbatim in Round 3 Probe Outputs below; reproduced here with pipe separators substituted for semicolons only, for table-parsing safety). The first attempt, the plan's 8-second setTimeout form, failed to capture a rest day — {cls: "", ariaDisabled: null, opacity: "1", ring: "none"} is document.activeElement reading as body, not a calendar day, proving nothing, and is logged as a failed capture rather than a result. A second attempt, using a focusin-listener form substituted because the timer form is fragile, returned: calendar-day calendar-day--rest ; aria-disabled: "true" ; opacity: "1" ; ring: "rgb(255, 255, 255) 0px 0px 0px 2px, rgb(252, 76, 2) 0px 0px 0px 4px" — matching expectation. Developer's verbatim words, both sub-checks: "1) yes 2. it reads less non-interactive (becomes slightly more lively (day number and \"-\") but it's very subtle and not a big issue for me)." Sub-check 1, ring legibility, is a clean yes. Sub-check 2, still reads as non-interactive, is recorded as observed and accepted rather than a silent pass: the fix un-dims the label along with the ring while focused, a real, accepted narrowing of the D-07 disabled affordance the developer judges subtle and not a concern. Both-theme confirmation: the same 'looks fine' answer given once to the explicit light/dark prompt spanning rows 14-17, with light checked first per instructions as the narrower margin. VERDICT: PASS. |
| 18. A focused control scrolled under the sticky top nav stays fully underneath it, on both `#/list` and `#/records`, and the Records jump bar still paints above a focused control elsewhere on the page | UI-02 | Paint order between a sticky header and a promoted focused element; no automated check in this repository can observe it. CR-01. | On `#/list` (Activities) and again on `#/records` (Records), Tab to a control partway down the page, then scroll so that control passes under the sticky top nav. The nav must stay opaque and on top — no part of the focused control or its ring may appear over the header. On Records, additionally use the sticky jump bar and confirm it still paints above a focused control elsewhere on the page; that ordering must survive the fix. Both themes, both routes. | **Round 3 2026-08-13:** The row's premise — a focused control scrolled under the sticky top nav on both Activities and Records — could not be exercised because the nav itself does not stay on screen. First exchange, developer's verbatim words: 'not sure what to look for in list. On records the "superlatives - pr tables - etc..." (is that the navbar?) that scrolls down with us andthe data scrolls behind it.' Second exchange, after being given a concrete recipe (Tab onto a column-header sort button on Activities, then scroll it under the top nav), developer's verbatim words: 'not sure what's supposed to happen in /list? If i scroll down the navbar disappears upwards. In records the ring stays put but the focus "superlatives" or "pr evolution" become orange as we reach that section. Not sure what you mean with opaque.' Probe D (see Round 3 Probe Outputs, Probe D), run to test this row's own premise on Activities, returned parent HEADER#app-nav-root, navH 77, parentH 77, navPos sticky, parentPos static — navH equals parentH exactly. What the developer described on Records (the jump-bar links reading "superlatives"/"pr evolution" turning orange as each section is reached) is the jump bar's own active-section highlighting, not a focus ring, and the jump-bar-above-content paint ordering was separately observed intact in the first exchange ('the data scrolls behind it'). This row could not be observed in either light or dark theme, because its premise — the nav remaining on screen long enough to scroll a focused control under it — failed on Activities before any themed comparison was possible. VERDICT: FAIL. |
| 19. The no-regression re-confirmation: the detail view's two-option x-axis toggle, the input/select/textarea baselines, the panel/card rhythm, and the Activities row-click model all read unchanged | UI-01, UI-02, UI-03, ACT-01 | The three fixes touch a global stacking ladder, a shared option class and a global disabled rule, so the things that must look unchanged need their own verdict rather than an assumption. | Four sub-checks, each needing its own answer: (a) the detail view's two-option x-axis toggle, Distance / Time, still shows rounded outer corners and a square joint, unchanged from 19-09; (b) the input, select and textarea baselines across Activities and Calendar still render their intended border, padding, background and 4px corners, with no unstyled browser default; (c) the card and panel rhythm on Overview and Records reads as it did; (d) an Activities row click still navigates to the detail view. | **Round 3 2026-08-13:** The no-regression re-confirmation covers four sub-checks on the developer's single verbatim response, 'all look good and unchanged': (a) the detail view's two-option x-axis toggle, Distance / Time, keeps its rounded outer corners and square middle joint, unchanged from 19-09; (b) the input, select and textarea baselines across Activities and Calendar still render bordered, padded, 4px-cornered boxes with no unstyled browser default; (c) the card and panel rhythm on Overview and Records still reads as one rhythm, unchanged; (d) an Activities row click still navigates to the detail view, the reference interaction pattern Phase 20 depends on. No probe underlies this row; it rests on the developer's direct visual confirmation of all four sub-checks together, covering UI-01, UI-02, UI-03 and ACT-01's confirmed-unregressed evidence. VERDICT: PASS. |

**Rows 20-24 below are Round 4 (plan 19-17).** Rows 20 and 21 supersede row 18's Round 3 FAIL
in the same way rows 14-19 already supersede the file's own earlier "Next step" and Approval
sections rather than replacing them — row 18 stays exactly as recorded above, permanent
history, byte-identical. Each of rows 20-24 requires its own independent PASS/FAIL verdict
token in its own Observation cell, exactly as rows 14-19 did; a single verdict covering more
than one row is not sufficient to close any of them. Rows 20-23 additionally require both
themes recorded explicitly in their own Observation cell; row 24 is the no-regression row and
is deliberately excluded from that requirement, matching the Round 3 precedent that excluded
row 19 the same way. Two probes gate this round's checkpoint, and both must be run and
recorded, verbatim, before the rows that depend on them are judged on appearances. **Probe G**
is plan 19-14's Probe F snippet (`19-GAP7-DIAGNOSIS.md` § Fix confirmation (Round 4)), re-run
in this session on both routes before rows 20, 21 and 22 are judged — the letter advances
because this is an independent run recorded in a different file, not because the snippet
differs, so do not go looking for a snippet labelled G inside that file; there is none there,
and there should not be. **Probe H** is a new stacking-declaration read, defined in Task 2's
own instructions, run once before row 21 is judged. Each new Observation cell below carries
the Round 4 sentinel and nothing else until Task 2 fills it.

| Behavior | Requirement | Why Manual | Test Instructions | Observation |
|----------|-------------|------------|-------------------|--------------|
| 20. The top nav stays on screen while the page scrolls, on two routes | UI-02 | Whether a sticky element holds during a scroll is a rendered-layout fact; `styles.test.ts` runs in vitest's `node` environment with no DOM and no CSSOM, so no assertion in this repository can observe it. This is the row Round 3's Probe D disproved; the developer's own words then were that the navbar disappears upwards. | On `#/list` (Activities), scroll to the top, then scroll down a full screen or more, slowly — the top nav must remain visible, pinned against the top of the window, while the content moves behind it. Then the same on `#/records`. Both themes, both routes. | **Round 4 2026-08-13:** Probe G run on both routes before this row was judged (see Round 4 Probe Outputs) — `#/list`: `{"route":"#/list","sy":600,"rootTop":0,"navTop":0,"rootPos":"sticky","navPos":"static"}`; `#/records`: `{"route":"#/records","sy":600,"rootTop":0,"navTop":0,"rootPos":"sticky","navPos":"static"}` — both meet the stated PASS conditions (`sy` 600 >= 400; `rootTop`/`navTop` both 0). The developer answered "pass all" to the four enumerated steps relayed in order — Activities light, Activities dark, Records light, Records dark: the top nav stayed pinned against the top of the window on both routes, in both themes, while the content scrolled behind it. VERDICT: PASS |
| 21. A focused control scrolled under the nav stays underneath it (the original CR-01 paint-order question, never before observable) | UI-02 | Paint order between a sticky header and a promoted focused element; no automated check in this repository can observe it, and this premise has never before been reachable — row 18 failed on its own premise before this question could even be asked. | On `#/list`, Tab to a control partway down the page — a column-header sort button, a filter control, a pagination button — then scroll so that focused control travels up and passes beneath the nav. The nav must stay opaque and on top; no part of the focused control or its orange ring may appear over the header. Repeat on `#/records`. Both themes, both routes. | **Round 4 2026-08-13:** Probe H run once before this row was judged (see Round 4 Probe Outputs): `{"position":"sticky","zIndex":"20","navContainsLinks":true}` — the nav's sticky declaration and its `z-index: 20` are confirmed live in this session's bundle, and the nav element contains its links. The developer answered "pass all" to the four enumerated steps relayed in order — Activities light, Activities dark, Records light, Records dark: on `#/list` (Activities) and again on `#/records`, a focused control's orange ring travelled up and passed beneath the nav in every route/theme combination, and the nav stayed opaque and on top — no part of the control or its ring painted over the header. This is the first time the original CR-01 paint-order question has ever been observed on a rendered page. VERDICT: PASS |
| 22. The Records jump bar still paints above a focused control elsewhere, and is still parked correctly beneath the nav | UI-02 | The jump bar's `top` is computed at runtime from the live rendered nav height, so a change to the nav's sticky rung can mis-park it, and its paint ordering against a focused control is not observable automatically. | On `#/records`, confirm the jump bar sits flush beneath the nav with no gap and no overlap while scrolling; click a jump link, then Tab to a control in the section below it and confirm the jump bar still paints above that focused control. Both themes. | **Round 4 2026-08-13:** On `#/records`, checked against two separate sub-questions in both themes. The developer's first answer was a single blanket "pass" — that blanket answer was explicitly refused as insufficient because it did not confirm which theme(s) or which of the two sub-questions it covered, and the developer was re-asked directly; confirmed verbatim: "yes both themes, both checks". Recorded as two independent theme verdicts rather than compressed into one: light theme PASS on (a) the jump bar sitting flush beneath the nav with no gap and no overlap while scrolling, and (b) the jump bar still painting above a focused control in the section below it after clicking a jump link; dark theme PASS on both (a) and (b) as well. VERDICT: PASS |
| 23. A focused nav link's own ring is fully visible | UI-02 | The ring on a descendant of the sticky rung is contained by that rung's stacking context, which this project has not measured; the code review marked the claim inferred-only. | From the top of any route, Tab until a nav link is focused. Its orange ring must be fully visible on all four sides, not clipped by the nav's bottom border and not painted over by the nav's own background. Both themes. | **Round 4 2026-08-13:** From the top of a route, Tab until a nav link itself is focused. The developer answered "pass both" — light theme PASS, dark theme PASS: the focused nav link's own orange ring is fully visible on all four sides in both themes, not clipped by the nav's bottom border and not painted over by the nav's own background. VERDICT: PASS |
| 24. The no-regression re-confirmation, this round's version | UI-01, UI-02, UI-03, ACT-01 | This round moved a sticky rung, rewrote guard assertions and edited comments across the stylesheet, so the things that must look unchanged need their own verdict rather than an assumption. **Not theme-gated** — every sub-check below is a geometry, structure or navigation claim rather than a colour one, matching the Round 3 precedent where row 19 was deliberately excluded from the both-theme requirement while rows 14-18 carried it. | Five sub-checks, each needing its own answer: (a) the detail view's two-option x-axis toggle, Distance and Time, still shows rounded outer corners and a square joint; (b) the input, select and textarea baselines across Activities and Calendar still render their intended border, padding, background and 4px corners, none reading as a raw browser default; (c) the card and panel rhythm on Overview and Records reads as it did; (d) an Activities row click still navigates to the detail view; (e) the nav itself looks unchanged — same height, same padding, same border beneath it, links in the same place. | **Round 4 2026-08-13:** Five sub-checks, each answered separately by the developer verbatim: "a. pass b. pas c. pass d. pass e. pass" — (a) the detail view's two-option x-axis Distance/Time toggle still shows rounded outer corners and a square joint, PASS; (b) inputs across Activities and Calendar remain bordered, padded, 4px-cornered, none reading as a raw browser default, PASS; (c) the card and panel rhythm on Overview and Records reads as it did, unchanged, PASS; (d) an Activities row click still navigates to the detail view, PASS; (e) the nav itself looks unchanged — same height, same padding, same border beneath it, links in the same place, PASS. This row is UI-01, UI-03 and ACT-01's confirmed-unregressed evidence for this round, each backed by its own lettered sub-check rather than a blanket approval. VERDICT: PASS |

### Round 3 Probe Outputs

Recorded fully verbatim and unaltered from the developer's session at `http://localhost:8099/strava-widgets/` (Safari/WebKit), 2026-08-13, across two exchanges. Referenced by rows 14-18 above. Only the Manual-Only Verifications table cells substitute `;` for `|` inside these outputs, where noted, for table-parsing safety; the outputs below are the unaltered source.

**Probe A — segmented corner geometry.** First run, on the Trends default view before switching tabs, returned only two groups — incomplete, not usable for judgment. Re-run after switching to the Training Load tab, full result:

```
[["Trends views", ["Volume => 4px 0px 0px 4px", "Year-over-Year => 0px", "Cadence & HR => 0px", "Training Load => 0px", "Gear => 0px 4px 4px 0px"]], ["Volume granularity", ["Weekly => 4px 0px 0px 4px", "Monthly => 0px", "Yearly => 0px 4px 4px 0px"]], ["TRIMP model", ["Edwards => 4px 0px 0px 4px", "Banister => 0px 4px 4px 0px"]], ["Training load window", ["3mo => 4px 0px 0px 4px", "12mo => 0px", "All => 0px 4px 4px 0px"]]]
```

Expected: first option `4px 0px 0px 4px`, last option `0px 4px 4px 0px`, every middle option exactly `0px`. Matches expectation for all four groups.
...
Expected: `"20"`. Matches expectation — the CR-01 `z-index: 20` declaration is confirmed present in the shipped bundle.

**Probe C — focus ring on a disabled control.** First attempt, the plan's 8-second `setTimeout` form, failed to capture a rest day:

```
{cls: "", ariaDisabled: null, opacity: "1", ring: "none"}
```

That is `document.activeElement` reading as `<body>`, not a calendar day — the reported opacity is the body's and proves nothing. Recorded as a failed capture, not as a result.

Second attempt, using a focusin-listener form substituted by the orchestrator because the timer form is fragile:

```
calendar-day calendar-day--rest | aria-disabled: "true" | opacity: "1" | ring: "rgb(255, 255, 255) 0px 0px 0px 2px, rgb(252, 76, 2) 0px 0px 0px 4px"
```

Expected: class contains `calendar-day--rest`, `aria-disabled` is `"true"`, `opacity` is `"1"` (not `"0.6"`), and `ring` is a non-empty two-stop box-shadow. Matches expectation.

**Probe D — nav stickiness (NEW, added mid-session, not in the plan).** Added by the orchestrator after the developer reported the nav scrolling away, to test row 18's own premise before judging the row:

```
{parent: "HEADER#app-nav-root", navH: 77, parentH: 77, navPos: "sticky", parentPos: "static"}
```

`navH` equals `parentH` exactly (77 = 77) — an observation, not an explanation of mechanism.

### Round 4 Probe Outputs

Recorded fully verbatim from the developer's session, dated 2026-08-13, against the same
`http://localhost:8099/strava-widgets/` staging as Task 1, independently verified by the
orchestrator before any observation was judged: the served bundle is byte-identical to the
local `dist/widgets` build and contains `#app-nav-root{position:sticky;top:0;z-index:20}`, and
the staging symlink `/tmp/gh-pages/strava-widgets` resolves to
`/Users/pedf/workspace/strava-widgets/dist/widgets`. Referenced by rows 20, 21 and 22 above.
Steps were relayed to the developer in six groups, in order, with each group completed before
the next was asked.

**Probe G — does the nav actually hold? Run on `#/list` and again on `#/records`, before
judging rows 20, 21 or 22.** This is plan 19-14's Probe F snippet, re-run in this session:
...
Expected: `sy` around 600 (not under 400, or the run does not count), and the nav's `top` value
at 0 or within a pixel of it. Both runs match expectation exactly: `sy` is 600 on both routes,
`rootTop`/`navTop` are both `0`, `rootPos` is `"sticky"` and `navPos` is `"static"` — the sticky
rung sits on `#app-nav-root` exactly as plan 19-14 moved it, and the element carrying it stayed
pinned to the top of the window through a full 600px scroll on both routes. No re-run was
needed on either route.

**Probe H — is the nav's stacking declaration live in this bundle? Run once, before judging row
21.**
...
Expected: `sticky`, `"20"`, and `true`. Matches expectation exactly — the `z-index: 20`
declaration this round's ladder comment describes is confirmed present and live in the bundle
this session actually observed, not merely present in source text.
...
- [ ] **Round 3 2026-08-13 (resolution):** PARTIAL. Rows 14, 15, 16, 17 and 19 each carry an independent PASS verdict — CR-02 and CR-03 are confirmed closed on rendered evidence (Probes A and C). Row 18 carries FAIL: the sticky nav does not remain on screen while scrolling on Activities (Probe D, GAP 7), a newly-found defect distinct from CR-01's z-index ordering, whose own `z-index: 20` declaration Probe B confirms is present in the shipped bundle (GAP 6) even though its rendered effect against the original paint-order question remains unconfirmed. `nyquist_compliant` remains `false`. See GAP 4/GAP 5 (resolved) and GAP 6/GAP 7 (not resolved) below, and the Round 3 addendum to the Approval line.
- [x] **Round 4 2026-08-13 (resolution):** `nyquist_compliant` set back to `true`. All five Round 4 rows (20-24) carry an independent PASS verdict, and both Round 4 probes (G and H) returned their expected values before the rows that depend on them were judged. Row 18's Round 3 FAIL stays exactly as recorded — permanent history, not rewritten, not counted against this round's row-scoped evaluation. GAP 7 and GAP 6 are both resolved this round on rendered evidence (row 20 and row 21 respectively); GAP 8 is newly opened, unpatched, and argued below not to block UI-02. See the Round 4 Approval addendum and the Gap-Closure Record.
...
**Round 3 Approval addendum (2026-08-13) — supersedes the "closed" reading of the paragraph above; the paragraph itself is left intact as history, not replaced.** PARTIAL. Five of the six new agenda rows carry an independent PASS verdict (14, 15, 16, 17, 19); row 18 carries FAIL — its premise, a focused control scrolled under an on-screen sticky nav, could not be exercised because the nav itself does not remain on screen while scrolling on Activities (Probe D: `navH` 77 equals `parentH` 77). `nyquist_compliant` remains `false`. This session ran entirely at `http://localhost:8099/strava-widgets/` (Safari/WebKit), across two exchanges, per Task 1's staging. See GAP 4 (CR-02, resolved), GAP 5 (CR-03, resolved), GAP 6 (CR-01's declaration confirmed shipped, effect unconfirmed) and GAP 7 (the newly-found nav-stickiness defect, not resolved) below.

**requirements-completed (Round 3, this checkpoint):**

- **CONFIRMED (unregressed, not newly completed — each was already Complete):** `UI-01` (row 19 sub-check (b): input/select/textarea baselines across Activities and Calendar, per the developer's "all look good and unchanged"), `UI-03` (row 19 sub-check (c): card/panel rhythm on Overview and Records), `ACT-01` (row 19 sub-check (d): Activities row click still navigates to the detail view).
- **NOT COMPLETED:** `UI-02` — row 18 (FAIL) blocks it; see GAP 7. Another `/gsd-plan-phase 19 --gaps` pass is required before this requirement can close.

**Round 4 Approval addendum (2026-08-13) — supersedes the "PARTIAL" reading of the Round 3 addendum above; that paragraph is left intact as history, not replaced.** CLEAN SWEEP. All five Round 4 agenda rows (20-24) carry an independent PASS verdict: row 20 (the nav holds, on both routes, both themes — Probe G), row 21 (the original CR-01 paint-order question, finally observed — Probe H plus both routes, both themes), row 22 (the Records jump bar's parking and paint order, both themes, with a refused blanket verdict re-asked and re-confirmed per-theme), row 23 (a focused nav link's own ring, both themes) and row 24 (the five-part no-regression sweep). Both Round 4 probes returned their expected values before any dependent row was judged. This session ran against `http://localhost:8099/strava-widgets/`, independently verified byte-identical to the local `dist/widgets` build before any observation was made. `nyquist_compliant: true`. One new, unpatched defect was found unprompted during this session and is recorded as GAP 8 below — it is argued, not assumed, not to block UI-02; see the Gap-Closure Record.

**requirements-completed (Round 4, this checkpoint):**

- **CONFIRMED (UI-02 newly complete on this round's clean sweep):** `UI-02` — rows 20-24 all PASS, closing GAP 7 (row 20) and GAP 6 (row 21) on rendered evidence. Combined with rows 3-6, 10-13 (already clean since 19-05/19-09) and rows 14-17 (clean since Round 3), every row this requirement's own row→requirement map names is now PASS.
- **CONFIRMED (unregressed, not newly completed — each was already Complete):** `UI-01` (row 24 sub-check (b): input/select/textarea baselines across Activities and Calendar), `UI-03` (row 24 sub-check (c): card/panel rhythm on Overview and Records), `ACT-01` (row 24 sub-check (d): an Activities row click still navigates to the detail view) — each backed by its own lettered sub-check verdict, not a blanket approval.
- **NOT COMPLETED:** none. The phase gate closes on this round.
...
### GAP 4 — Segmented middle-option radius (CR-02)

- **Rows blocked:** 14, 15, 16
- **Requirement blocked:** UI-02
- **Decisions implicated:** D-05 (button baseline), D-10 (segmented clipping/end-child radius)
- **Fact as observed:** `19-VERIFICATION.md` found the `button` baseline's `border-radius: var(--radius-control)` reaching every `.segmented__option` with no cancelling rule, so middle options of the three live 3+-option Trends segmented groups (the 5-option tab list, the Volume-granularity group, the Training-load window group) rendered fully rounded instead of square-jointed.
- **Why the automated gate did not catch it:** no test in `styles.test.ts` asserted `.segmented__option`'s own `border-radius`, only the end-child rules; a green suite proves a rule exists in the source, not that a sibling rule fails to cancel it.
- **Resolution 2026-08-13:** CLOSED. Plan 19-11 added `border-radius: 0` to `.segmented__option`, pinned by an anchored source-level assertion watched failing before the fix landed. Rendered effect confirmed this round by Probe A (Round 3 Probe Outputs), run on the Trends default view then re-run on the Volume and Training Load tabs after the first run returned only two groups: `[["Trends views", ["Volume => 4px 0px 0px 4px", "Year-over-Year => 0px", "Cadence & HR => 0px", "Training Load => 0px", "Gear => 0px 4px 4px 0px"]], ["Volume granularity", ["Weekly => 4px 0px 0px 4px", "Monthly => 0px", "Yearly => 0px 4px 4px 0px"]], ["TRIMP model", ["Edwards => 4px 0px 0px 4px", "Banister => 0px 4px 4px 0px"]], ["Training load window", ["3mo => 4px 0px 0px 4px", "12mo => 0px", "All => 0px 4px 4px 0px"]]]` — every middle option computes exactly `0px`, matching expectation. Rows 14, 15 and 16 each carry an independent PASS verdict this round.

### GAP 5 — Focus-ring opacity on focusable disabled controls (CR-03)

- **Row blocked:** 17
- **Requirement blocked:** UI-02
- **Decisions implicated:** D-07 (disabled treatment), D-09 (two-tone focus ring), D-11 (contrast discharge)
- **Fact as observed:** `19-VERIFICATION.md` found `opacity: 0.6` on `[aria-disabled="true"]` compositing the `:focus-visible` ring on calendar rest days (real, focusable buttons carrying only `aria-disabled`), dropping computed non-text contrast to roughly 2.19:1 light / 2.93:1 dark, both under the 3:1 SC 1.4.11 floor the file's own comment claims cleared.
- **Why the automated gate did not catch it:** no test asserted the opacity/focus-ring interaction on a disabled-but-focusable element; the base disabled rule and the ring rule were each individually tested but never together.
- **Resolution 2026-08-13:** CLOSED. Plan 19-11 added `:disabled:focus-visible, [aria-disabled="true"]:focus-visible { opacity: 1 }`, pinned by an anchored source-level assertion watched failing before the fix landed. Rendered effect confirmed this round by Probe C's second attempt (Round 3 Probe Outputs) — the first attempt (the plan's 8-second `setTimeout` form) failed to capture a rest day at all, reading `document.activeElement` as `<body>`, and is logged as a failed capture, not a result. The second attempt, a focusin-listener form substituted because the timer form is fragile, returned: class contains `calendar-day--rest`, `aria-disabled` is `"true"`, `opacity` is `"1"` (not `"0.6"`), and `ring` is a non-empty two-stop box-shadow (`rgb(255, 255, 255) 0px 0px 0px 2px, rgb(252, 76, 2) 0px 0px 0px 4px`) — matching expectation. The developer separately confirmed the day still reads as non-interactive while focused, verbatim: "it reads less non-interactive (becomes slightly more lively (day number and \"-\") but it's very subtle and not a big issue for me)" — recorded as an observed and accepted narrowing of the D-07 disabled affordance, not a silent pass. Row 17 carries an independent PASS verdict this round.

### GAP 6 — Sticky-nav paint order fix (CR-01): declaration shipped, rendered effect unconfirmed

- **Row blocked:** 18
- **Requirement blocked:** UI-02
- **Decisions implicated:** D-09 (two-tone focus ring, `position: relative; z-index: 1`), D-10 (stacking/clipping fixes), D-12 (`:focus-visible` kept global and unscoped)
- **Fact as observed:** `19-VERIFICATION.md` found `:focus-visible`'s `z-index: 1` (added by plan 19-07 to fix the prior round's GAP 2) has no counterpart `z-index` on the sticky `.app-nav`, so a focused control scrolled under the header could paint over it. Plan 19-10 gave `.app-nav` an explicit `z-index: 20` above the ring, as a written four-rung stacking-layer ladder, pinned by a numeric assertion watched failing against the real shipped defect before the fix landed.
- **Why the automated gate did not catch the original defect:** no test asserted `.app-nav`'s own `z-index` against `:focus-visible`'s; each rule was tested in isolation.
- **Status 2026-08-13 — NOT resolved.** Probe B (Round 3 Probe Outputs), run once before row 18 was judged, returned `20` — confirming the `z-index: 20` declaration is present in the shipped bundle exactly as plan 19-10 wrote it. But row 18 is recorded FAIL this round: the developer reported the nav itself does not remain on screen while scrolling, a premise failure that made the z-index ordering unobservable one way or the other. The declaration is shipped; its rendered effect against the original CR-01 paint-order question is unconfirmed. This entry is superseded by GAP 7 below, which records the newly-discovered defect that actually blocked row 18 this round, rather than being marked resolved.
- **Resolution (Round 4, 2026-08-13): CLOSED.** Row 21, gated on Probe H confirming the `z-index: 20` declaration is live in this session's bundle (`{"position":"sticky","zIndex":"20","navContainsLinks":true}`), is the first observation of the original CR-01 paint-order question ever recorded on a rendered page. Developer's relayed answer, verbatim: "pass all" — to the four enumerated steps covering Activities light, Activities dark, Records light, Records dark, each asking whether any part of a focused control or its orange ring painted over the nav as it scrolled beneath it. None did, in any of the four route/theme combinations. The declaration this gap tracks is now confirmed both shipped and effective.

### GAP 7 — Sticky nav does not remain on screen while scrolling (new, found during Round 3)

- **Row blocked:** 18
- **Requirement blocked:** UI-02
- **Decisions implicated:** D-09 (two-tone focus ring, `position: relative; z-index: 1`), D-10 (stacking/clipping fixes), D-12 (`:focus-visible` kept global and unscoped)
- **Verbatim developer observations:**
  - First exchange: "not sure what to look for in list. On records the \"superlatives - pr tables - etc...\" (is that the navbar?) that scrolls down with us andthe data scrolls behind it."
  - Second exchange, after being given a concrete recipe (Tab onto a column-header sort button on Activities, then scroll it under the top nav): "not sure what's supposed to happen in /list? If i scroll down the navbar disappears upwards. In records the ring stays put but the focus \"superlatives\" or \"pr evolution\" become orange as we reach that section. Not sure what you mean with opaque."
- **Probe D, verbatim (Round 3 Probe Outputs), run on Activities to test this row's own premise before it was judged:** `{parent: "HEADER#app-nav-root", navH: 77, parentH: 77, navPos: "sticky", parentPos: "static"}` — `navH` equals `parentH` exactly (77 = 77).
- **Fact as observed:** row 18's premise — a focused control scrolling under a sticky, on-screen top nav — could not be exercised on Activities: the developer reports the nav scrolling away rather than staying put, and Probe D's `navH`/`parentH` equality is recorded as-is. Probe B separately confirmed the CR-01 `z-index: 20` declaration is present in the shipped bundle (see GAP 6), so this is a distinct, newly-found defect rather than a recurrence of CR-01. On Records, the developer's report of jump-bar links ("superlatives"/"pr evolution") turning orange as each section is reached is the jump bar's own active-section highlighting, not a focus ring; the jump-bar-above-content paint ordering was separately observed intact in the first exchange ("the data scrolls behind it").
...
**Resolution (Round 4 checkpoint, 2026-08-13): CLOSED.** Plan 19-14 applied the H1-prescribed
fix (moving the sticky rung to `#app-nav-root`); this plan's row 20 is the first re-verification
of that fix on rendered evidence gathered independently in this session, gated on Probe G run on
both routes before the row was judged: `#/list` returned
`{"route":"#/list","sy":600,"rootTop":0,"navTop":0,"rootPos":"sticky","navPos":"static"}` and
`#/records` returned
`{"route":"#/records","sy":600,"rootTop":0,"navTop":0,"rootPos":"sticky","navPos":"static"}` —
both meeting the stated PASS conditions. Developer's relayed answer, verbatim: "pass all" — to
the four enumerated steps covering Activities light, Activities dark, Records light, Records
dark, each asking whether the top nav stayed pinned to the top of the window while the page
scrolled a full screen or more beneath it. It did, on every route/theme combination. The
premise that blocked row 18 in Round 3 no longer fails.

### GAP 8 — Leaflet map tiles paint over the nav; the ladder comment's totality claim is now known incomplete (new, found during Round 4)

- **Row blocked:** none of rows 20-24 — this was reported unprompted, during Group C (row 21), not in response to any agenda item.
- **Requirement implicated:** UI-02 (argued below, not to block it) — see the Round 4 Approval addendum.
- **Decisions implicated:** D-09, D-10, D-12 (the sticky-layer ladder), and by extension plan 19-16's ladder-comment truth-repair work.
- **Verbatim developer observation, reported unprompted:** "By the way I noticed that the map tiles go over the navbar. That's the only weird thing i noticed. I really don't care about this so if it's more than just a very quick fix you can skip it."
- **Analysis (source-derived, marked as reasoning, not measurement — no probe was run against this defect).** Leaflet ships its own stacking values: panes at `z-index: 400`, controls at `z-index: 1000` (Leaflet's own CSS, not this project's). `grep -n "leaflet" src/dashboard/styles.css` returns nothing — this project's stylesheet does not declare, override or otherwise touch Leaflet's own layering at all. `#app-nav-root` carries `z-index: 20` (this round's ladder rung 4, the highest rung this project declares), which is far below Leaflet's panes. This is consistent with, and sufficient to explain, what the developer observed — but it was read from source and Leaflet's own stylesheet, not confirmed by a computed-style probe against the rendered map, and is recorded here as reasoning rather than as a measured fact.
- **Second face — a truth defect in freshly-shipped work.** Plan 19-16 shipped the sticky-layer ladder comment (`src/dashboard/styles.css`, immediately above `#app-nav-root`) presenting a **totally-ordered, four-rung** ladder — `#app-nav-root` at rung 4 (top), `.records-jump` at rung 3, `.splits-table__km` at rung 2, `:focus-visible` at rung 1 (bottom) — stated as "the single, written statement of that order." Leaflet's panes (`z-index: 400`) and controls (`z-index: 1000`) sit above all four of this project's own rungs and are not listed anywhere in that comment. The comment's claim to totality is therefore now known to be incomplete: a fifth, unlisted rung exists above the top of the stated ladder. This is precisely the class of defect plan 19-16 was written to eliminate (R3-WR-03/WR-04, stacking-context claims that outran what had actually been measured) — recorded here as a defect in that same freshly-shipped work, not softened or minimized.
- **Why the automated gate did not catch it:** no test in this repository asserts anything about Leaflet's own stacking values, and no row on any round's checkpoint agenda before this one ever named the map view against the nav specifically — GAP 8 was found by the developer noticing it unprompted, not by any agenda item designed to look for it.
- **Not patched.** Per the phase's house rule since 16-09, no fix is applied under checkpoint pressure — this is exactly the pattern that produced four earlier false-green mechanisms (see Lessons below). The developer was told this would be recorded and dispositioned after the phase closes, either as a small follow-up plan (a z-index fix on the map container, one browser re-check, and a correction to the ladder comment's totality claim) or as a deferred item. That disposition is the user's call, not this plan's — see `deferred-items.md` / a future todo for the actual decision.
- **Resolution:** NOT resolved. No suggested fix and no root-cause remediation is recorded beyond the source-derived analysis above, per the plan's rule for a newly-found, still-open gap.
...
**Round 3 addendum (2026-08-13).** The developer was asked twice, explicitly, for a yes/no on each of the three items above and did not report on any of them during this session. Recorded as not reported in the Round 3 session for all three — not as resolved, and not as still-occurring.

**Round 4 addendum (2026-08-13).** All three items were asked about this session and answered directly, verbatim, against the current code (not a stale build — see the orchestrator's independent verification at the top of Round 4 Probe Outputs).

1. **JavaScript SyntaxError on every page load — STILL PRESENT, now without a file or line reference.** Verbatim: "javascript sytexerror still appears but doesnt mention file or line." Previously (Round 3) it was not reported on; this session confirms it still occurs, and that it now surfaces with even less localizing information than before (no file or line at all, versus the earlier "can't see any line or file, doesnt expand" report, which at least implied an expandable-but-empty entry).
2. **Records "View Activity" links — STATUS CHANGE, now observed working.** Verbatim: "'view activity' links work fine... not sure what you asking. I click and goes to activity. From records." This is a change from the status recorded in an earlier round (links not navigating on click). No plan in Round 4 (19-13 through 19-17) touched `src/dashboard/views/records.ts` or any Records markup — `git diff --name-only` for this round's plans is `src/dashboard/styles.css`, `src/dashboard/styles.test.ts` and this validation record only — so this plan makes no claim that any Round 4 work fixed it. The cause of the change is unestablished: the earlier "does not navigate" record may itself have been wrong (a stale build, a one-off click miss, or a Round 3-specific browser state), or something outside this phase's own file set genuinely changed. Recorded as an observed status change with the developer's verbatim words, not as a fix.
3. **Calendar month picker — STILL a plain wide text box, now inheriting the shared control styling.** Verbatim: "Yeah calendar month picker just a text box. Maybe more stylized now according to the rest of the theme but sitll just a wide text box." Unchanged in kind from the Round 3/19-05 record (Safari's `type="month"` degradation, `src/dashboard/views/calendar.ts:280`) — the only change is that it now picks up the shared input baseline's border/padding/radius styling, which is exactly what UI-01's control-styling work would be expected to do to any text-degraded native input, not a new defect.
...
3. **A checkpoint agenda that names one instance of a component class and generalises from it, paired with a blanket verdict that cannot be traced to any single observation.** The 19-09 checkpoint asked about the two-option detail-view segmented control and a general "buttons and selects" sweep; it never named the three-option Trends groups specifically, and its single blanket verdict — "Everything looks good. Approved." — covering four rows at once could not be traced back to any one of them. `19-VERIFICATION.md` reopened UI-02 on exactly this gap, finding CR-01/CR-02/CR-03 by source review after the checkpoint had already closed. **Mitigation now in place:** per-row verdict tokens (each of rows 14-19 requires its own independent PASS/FAIL), per-row content anchors (each row's own controls must be named in its own observation, asserted by the gate), both-theme requirements (rows 14-18 each require the words `light` and `dark` in their own observation), and probe-gated rows (objective computed-style/layout probes run and recorded before the rows that depend on them are judged). This is the third distinct false-green mechanism this phase has recorded.
4. **A defect confirmed only by source-code reading, and a fix for it verified the same way, neither ever checked against the rendered page until a probe run for an unrelated reason exposed the gap.** `19-REVIEW.md`/`19-VERIFICATION.md` diagnosed CR-01 — a focused control's `z-index: 1` painting over the sticky `.app-nav` — entirely from reading `styles.css`; plan 19-10's fix (`.app-nav { z-index: 20 }`) was likewise pinned only by a source-level numeric assertion. This Round 3 session's Probe B confirms the declaration is present in the shipped bundle exactly as written. But Probe D — run only because the developer reported the nav scrolling away, not because the plan anticipated it — found the nav does not remain sticky on screen at all on Activities (`navH` equalling `parentH`), a distinct defect the z-index-ordering diagnosis never considered, and one that makes the original z-index question unobservable in the one place it needed checking. This session's other mitigations did work as designed: Probe A caught that a whole segmented group (the Trends tab list) had genuinely never been probed for its corner geometry before this round, and Probe D caught that row 18's premise was false before the row could be judged on appearances alone. But this fourth mechanism is new: a chain of source-only diagnosis and source-only fix-verification can each be individually correct about what the source says and still miss whether the described condition is ever reached on the rendered page. No mitigation for this fourth mechanism exists yet; it is recorded here for a future phase to design one, rather than invented under this checkpoint's own pressure. **Running count after this session: four distinct false-green mechanisms recorded by this phase, not three — the plan's own drafted count predates this session's findings and is superseded by this note.**
5. **A guard documented as closing a hole it did not actually close, proven true only by mutation, not by reading.** `19-REVIEW-round3.md` found R3-CR-01 and R3-CR-02: two of this phase's own test assertions — a hover-rule guard and the stacking-order guard added for GAP 6/CR-01 — remained green even after the defect they were written to name was deliberately reintroduced into the stylesheet. Reading the assertion's source, the way every prior false-green in this phase was diagnosed, gave no signal that either guard was inert; only executing the mutation and watching the assertion fail to fail exposed it. This is a fifth distinct false-green mechanism, not a recurrence of mechanism 1 (a parse result a source assertion can't see) or mechanism 4 (a rendered condition source-only verification can't reach) — here the assertion's own logic, examined purely as text, looked correct and was not. **Mitigation now in place, from plan 19-15:** mutation-based acceptance criteria — a task's done-criteria requires a recorded terminal failure under a named, deliberately reintroduced mutation of the defect the assertion claims to guard, not merely the existence of an assertion that currently reads green. Seven mutations were executed and reverted against this phase's guard layer under that standard. **A second mitigation, from plan 19-13, addresses the companion risk of a plausible-but-wrong root-cause diagnosis:** a separate diagnosis plan, run before any fix is written, that must exclude every competing hypothesis by a cited, rendered-probe field rather than by argument, with an explicit INCONCLUSIVE stop available if no hypothesis clears that bar — used to confirm GAP 7's H1 root cause over four excluded alternatives (H2-H5) before plan 19-14 wrote a single line of fix. **Running count after this session: five distinct false-green mechanisms recorded by this phase — the Round 3 note's count of four is superseded by this note.**

## Next step

**Superseded by Round 3 (2026-08-13).** The "closed" statement above described the 19-09 outcome and is left in place as history, not deleted. As of Round 3, `nyquist_compliant: false` and the phase gate does **not** close: rows 14, 15, 16, 17 and 19 each carry an independent PASS verdict, closing CR-02 and CR-03 on rendered evidence, but row 18 carries FAIL — the sticky nav does not remain on screen, a newly-found defect (GAP 7) that blocks UI-02 independently of the CR-01 z-index fix's own (now-confirmed-shipped, per Probe B) declaration. `UI-01`, `UI-03` and `ACT-01` remain Complete, each with a Round 3 confirmed-unregressed note. `UI-02` remains open. **Another `/gsd-plan-phase 19 --gaps` pass is required** before this phase gate can close.

**Superseded by Round 4 (2026-08-13) — the phase gate closes.** All five Round 4 rows (20-24) carry an independent PASS verdict; GAP 7 (row 20) and GAP 6 (row 21) are both resolved on rendered evidence, gated on Probes G and H returning their expected values before any dependent row was judged. `nyquist_compliant: true`. `UI-02` is now ticked complete in `REQUIREMENTS.md` on this clean sweep — see the Round 4 Approval addendum above. `UI-01`, `UI-03` and `ACT-01` remain Complete, each with its own row 24 sub-check as this round's confirmed-unregressed note, not a blanket approval. One new gap was found unprompted this session (GAP 8: Leaflet map tiles paint over the nav, plus a totality defect in plan 19-16's own ladder comment) — argued above not to block UI-02's own text, left unpatched per house rule, and its disposition (small follow-up plan or deferred item) is left to the user after this phase closes. **No further gap-closure pass is required for Phase 19's own gate.**
