# Requirements Archive: v2.2 Pace Data Quality

**Archived:** 2026-09-19
**Status:** SHIPPED — 33/33 requirements complete (27 scoped 2026-09-08 + TD-01..TD-06 minted 2026-09-19 for Phase 31)
**Audit:** `milestones/v2.2-MILESTONE-AUDIT.md` (`passed`); the traceability table below is the final state, every row Complete.

For current requirements, see `.planning/REQUIREMENTS.md` (created fresh by the next `/gsd-new-milestone`).

**Requirement outcomes — validated as written unless listed here.** Each adjustment below was made
in place during the milestone with a dated provenance note (visible in the text that follows), and
each is a correction of a measured figure or an unsatisfiable wording, never a relaxation to fit:

- **PACE-06** — residual cohort corrected 13 → 14 (a gap-segment exclusion error undercounted it); the 14 are all marginal (≤2.44%) and enumerated in `26-RESIDUAL.md`, regenerated against the merged 1,899-activity archive (1,874 streams scanned).
- **PACE-04 / Criterion 1** — "strictly lower for all 154" amended to "strictly lower, or both exactly zero" (D-19): one activity's baseline fast mass is already 0.00%, making the literal reading unsatisfiable. 153 strictly improve, 1 ties at zero, 0 regress.
- **COV-01 / Criterion 3** — the stream-vs-`elapsed_time` comparison amended (D-06): `CanonicalStream` has no `elapsed_time`; the identity is over the stream's own span, and the metadata difference (3,394 vs 3,393 s on the exemplar) is reported, not absorbed.
- **QUAL-05 / Criterion 4** — "under ~5% (≈90 activities)" was jointly unsatisfiable with D-04's locked 154-activity decimation cohort (8.15% alone); split (`split-the-criterion`) into 4a, a failable gate, and 4b, the measured rate as a reported finding: 299 of 1,890 (15.8%). QUAL-05's "materially above ~5% is a calibration failure" wording was deliberately left in visible tension with the measured rate rather than dissolved.
- **ERA-02 / Criterion 5** — no-device-name cohort corrected 716/1,864 → 663/1,899 (34.9%), re-measured live over the merged index; the shortfall is better classification, not a defect.
- **PR-03 / PR-04 / PR-05** — "pending phase re-verification" wording replaced by the actual outcome (`28-VERIFICATION.md` `passed` 5/5, 2026-09-17); PR-03's pinned fixture value corrected 44.0 s / 9.09 m/s → 45.2 s / 8.85 m/s against the live `best-efforts.json`. PR-04 was signed four times as the diff regenerated (Round 4 bound to sha256 `97e1782c…`).
- **ELEV-01 / Criteria 1 & 3** — "34 closure-drift" was a raw start/end difference measured without the loop condition; loop-gated against a 100 m radius derived from the archive it is 21 (12 point-to-point + 1 no-position excluded by design, listed by id in `30-CALIBRATION.md`). The flagged union is 60 (3.2%) loop-gated vs the originally cited raw 71 (3.8%); both figures stay visible.
- **CUR-01..CUR-03** — validated as written. Note these are v2.2's *review-queue* CUR IDs; v2.1's CUR-01 (the exclusion tickbox, Phase 24) is a different requirement archived in `v2.1-REQUIREMENTS.md`.
- **TD-01..TD-06** — minted 2026-09-19 from the close-out audit, one per Phase 31 success criterion; all six closed with demonstrated-failing tests or two-run idempotence proofs.

**Not carried as debt:** the `## Future Requirements` section below (STREAM-05/06 re-derivation, CUR-04 queue dismiss) and `## Out of Scope` are the milestone's deliberate non-goals and remain valid inputs to the next milestone's scoping.
