...
**Phases:** 26-31 (6 phases; Phase 31 added 2026-09-19 from the close-out audit)
**Total Plans:** 69 (168 tasks)
**Requirements:** 33/33 satisfied (27 scoped + TD-01..TD-06)
**Milestone audit:** `passed` — `v2.2-MILESTONE-AUDIT.md` (final, 2026-09-19); preceded by `v2.2-INTERIM-AUDIT.md` (2026-09-17, phases 26-28) and `v2.2-CLOSEOUT-AUDIT-2026-09-18.md` (`tech_debt`, seeded Phase 31). All three sit beside this file.

> Archived from `.planning/ROADMAP.md` at the v2.2 milestone close. This file preserves the full
> phase details — goals, success criteria with their dated in-place corrections, wave breakdowns and
> per-plan checklists; the live ROADMAP.md keeps only the collapsed one-line-per-phase summary. For
> the shipped-milestone narrative and stats see `.planning/MILESTONES.md`; for requirement outcomes
> see `v2.2-REQUIREMENTS.md`.
>
> **Path note:** every reference to `.planning/v2.2-MILESTONE-AUDIT.md` in the archived text below,
> in `v2.2-REQUIREMENTS.md` and in the Phase 26-31 artifacts now resolves to
> `.planning/milestones/v2.2-MILESTONE-AUDIT.md`. The audit's own `supersedes` header explains that
> the 2026-09-18 close-out audit briefly carried the canonical filename before being renamed.
>
> **Structural note:** the live ROADMAP.md had Phase 31's detail block filed under `## Progress`
> (the `phase.add` misplacement). It is restored to its place after Phase 30 here.

## Overview

Make every pace figure the dashboard derives honest — one gap-aware derivation, plausibility guards
that actually bind, and per-activity quality signals you can see — without rewriting a single
committed stream. Scoped 2026-09-08 from a live investigation of all 1,864 committed streams rather
than a feature wish-list; executed against a 1,890-activity snapshot; merged with origin's
1,899-activity archive on 2026-09-19 before the final audit.

## Milestone Summary

**Inserted phase:**

- Phase 31: Tech-Debt Closure — Silent-Failure Guards & Docs Reconciliation (added 2026-09-19 from
  the close-out audit's ~30 advisory items; five silent-and-passing failures closed at the source
  with demonstrated-failing tests, every stale figure reconciled by regeneration, not hand edit)

**Key decisions:**

- Adaptive per-activity window (`max(20, 2.5 × p90(advance interval))`) over a fixed 20s — the
  fixed window manufactured the phantom fast mode the milestone existed to remove (Phase 26, D-16)
- Strict-priority gap classification (recording-gap > pause > covered) so coverage sums to an exact
  identity rather than an approximation (Phase 26, COV-01)
- Ceiling parameters measured live, not chosen: `CEILING_K = 1.28` (argmax 10k), minimum
  population 100 (order-statistic floor); derived only from the already-filtered population
  (Phase 28, PR-02)
- Demote-and-flag, never delete; every rejection retained as a visible effort through one shared
  demotion path (Phase 28, PR-03)
- Elevation is a sixth signal structurally excluded from the severe composite (Phase 30, D-06)
- ROADMAP Criterion 4's "under ~5%" split into a failable gate and a reported finding instead of
  retuning a locked threshold toward a stale target (Phase 27, `split-the-criterion`)
- Every phase ships a classifier-independent recount script and every generated artifact is proven
  idempotent by a second run (Phases 27-31)

**Issues resolved:**

- 26 CR-03: the pace chart band still derived under the fixed 20s window after rounds 1 and 2 passed
- 27 G-04: a list row lacking `quality` blanked the entire Activities list
- 28 CR-01: 13 owner-excluded over-ceiling efforts shipped `demotion: null`; recount and checkpoint
  both passed on the defect's own output
- 28 post-merge: one record (`3475730418@1mi`) changed hands on the merged archive; re-signed
- 31 TD-01..TD-06: see Phase 31 success criteria

**Issues deferred (recorded in ROADMAP § Phase 31 out-of-scope, MILESTONES.md and STATE.md):**

- 28 WR-06 opt-in ceiling-file write gate (developer decision on a D-07 mechanism)
- CUR-04 queue dismiss action; 26 F-26-02 histogram tails; 30 WR-02/WR-03 badge wording
- `index-client.ts` `ParsedDashboardIndexRow` retype (phase-sized; candidate for v2.3)
- STREAM-05/06 re-derivation from 1 Hz originals (gated on measuring whether quality signals justify it)
- STREAM-04 Garmin export adapter (externally blocked, inherited from v2.0)
