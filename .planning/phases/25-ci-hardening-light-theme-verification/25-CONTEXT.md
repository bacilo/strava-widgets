...
- **D-04:** Each round starts from **cleared site data** (or a fresh profile), and the write-up must quote `localStorage.getItem('dashboard-theme')` at the instant of observation, with the value being **`null` or `'auto'`** — never `'light'` or `'dark'`. Without this the row proves nothing: `theme.ts` reads the persisted mode *before* falling back to `prefers-color-scheme`, so a browser whose in-page control has been left on an explicit mode ignores the OS setting entirely — and a masked row looks identical to a passing one.
  - **AMENDED 2026-09-03, plan-checker blocker, approved by the developer.** As originally worded this decision demanded a literal `null`, which is **unreachable on a correctly-functioning page** and would have scored a working implementation BLOCKED. `src/dashboard/main.ts:29` calls `applyThemeMode(readStoredMode(resolveStorage()))` at module scope with no options, and `persist` defaults to `true` (`theme.ts:118`), so *every* load — including the first load of a freshly-cleared profile — writes `'auto'` into `dashboard-theme` before any DevTools read can happen. The inline bootstrap in `index.html:41` only reads (`getItem`), so it is not the writer; `main.ts` is.
  - The amendment preserves the decision's intent exactly rather than weakening it. What actually has to be true is that the in-page control is not overriding the OS, and that is a statement about `'light'`/`'dark'`, not about `null`: `parseThemeMode(null)` returns `'auto'` (`theme.ts:41`), and the live-follow listener gates on `readStoredMode(storage) === 'auto'` (`theme.ts:171`), which holds identically for `null` and for `'auto'`. The header toggle cycles `light → dark → auto` (`cycleThemeMode`, `theme.ts:55`), so a touched control that has been left on an explicit mode is caught by exactly this check.
  - This is the same defect class as D-05 and as Phase 24's R19/R26 — a row's own mandated setup emptying its discriminator — except in the opposite direction: here the row was unpassable rather than unfailable. Caught at plan-check time rather than after a burned round. **Like D-05, this amendment must be disclosed in the validation write-up, not quietly applied.**
