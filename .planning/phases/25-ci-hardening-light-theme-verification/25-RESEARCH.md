...
- **D-04 (AMENDED 2026-09-03 — see `25-CONTEXT.md` for the full amendment and its rationale):** Each round starts from **cleared site data** (or a fresh profile), and the write-up must quote `localStorage.getItem('dashboard-theme')` at the instant of observation, with the value being **`null` or `'auto'`** — never `'light'` or `'dark'`. Without this the row proves nothing: `theme.ts` reads the persisted mode *before* falling back to `prefers-color-scheme`, so a browser whose in-page control has been left on an explicit mode ignores the OS setting entirely — and a masked row looks identical to a passing one. **The original wording demanded a literal `null`, which is unreachable:** `main.ts:29` calls `applyThemeMode(...)` at module scope with `persist` defaulting to `true` (`theme.ts:118`), so every load writes `'auto'`. Since `parseThemeMode(null) === 'auto'` (`theme.ts:41`) and live-follow gates on `readStoredMode() === 'auto'` (`theme.ts:171`), the two values are behaviourally identical and the amendment preserves the intent.
...
**How to avoid:** D-04 already specifies the fix — start from cleared site data (or a fresh browser profile) and quote `localStorage.getItem('dashboard-theme')` at the instant of observation, for every round, requiring the value to be **`null` or `'auto'`** and never `'light'`/`'dark'` (AMENDED 2026-09-03; the original literal-`null` form is unreachable because `main.ts:29` persists `'auto'` on every load — see `25-CONTEXT.md` D-04).
**Correction to this pitfall's own framing:** "any prior visit" above is too strong. A prior visit alone writes `'auto'`, which still follows the OS. What actually masks the OS is the toggle being *left on* `'light'` or `'dark'` — the check is on the stored value, not on whether the browser was used before.
