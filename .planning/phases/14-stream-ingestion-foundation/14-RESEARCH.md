...
**Locked clarification — STREAM-01's "pace" channel is intentionally NOT a stored array.** The roadmap wording lists "(time, distance, pace, HR, cadence, elevation)". Pace is `Δd/Δt`, fully recoverable from the stored `t` and `d` arrays at zero storage cost; persisting it duplicates ~1,300-2,500 floats per activity for no capability gain and creates a second place normalization bugs can hide (see Anti-Patterns). The requirement is satisfied by *pace being derivable*, computed downstream in Phase 15/16, not by a `pace` key in the committed file. This is a recorded, intentional reading of STREAM-01 — not a silent scope reduction. Enforced by plan 14-01 Task 1, whose acceptance criteria assert `stream.types.ts` contains no `pace` identifier.
...
1. **Who owns the "already-archived intervals-activity, no original, no stream" reconciliation — backfill CLI or a new one-time script?**
   ...
   - **RESOLVED (planning, 2026-08-10):** Recommendation adopted. Folded into the backfill CLI as a third branch — `14-05-PLAN.md` Task 1 adds the live-API reconciliation branch over `data/activities/*.json` entries with `source_provider === 'intervals'` and no `data/streams/<id>.json`, fetching via `IntervalsClient.getAllStreams`, deriving through the same `derive-stream.ts` seam, and settling a 404 (past the ~1yr retention window) as reason code `no-original`. No fourth CLI command was added. Re-verify the exact orphan list at execution time rather than trusting this document's "2" count — it grows nightly.
...
2. **Manifest reason-code taxonomy: is "manual" sufficient for all 23 confirmed manual entries, or does the schema need to distinguish "genuinely no recording ever existed" from "recording existed but wasn't in the export"?**
   ...
   - **RESOLVED (planning, 2026-08-10):** Recommendation adopted. `manual` covers all 23 confirmed entries. The taxonomy is implemented as an extensible closed set by `classifyUnavailable` in `14-03-PLAN.md` (`manual` | `no-original` | `no-samples`), with `treadmill` deliberately NOT implemented — zero activities in the archive need it today, and adding it later is a one-line addition to the union plus a branch in `classifyUnavailable`. A future Garmin export (STREAM-04, deferred) that retroactively supplies a recording would flip an entry from `available: false` to `available: true` via the existing idempotent manifest upsert; no schema change required.
