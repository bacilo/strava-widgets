...
| 3. Buttons and selects read as one treatment after the low-specificity `button` baseline cascades under 12 existing classes | UI-02 | Cascade outcome is a computed-style question; there is no CSSOM. | Compare all 12 button treatments across the five screens. Confirm none regressed. | Developer's verbatim response: "Everything else seems fine". **BLOCKED BY DEFECT — not a pass.** The same dead `--radius-control` token (see row 1) is referenced by the `button` baseline at `src/dashboard/styles.css` line 1201, so every one of the 12 compared button treatments was rendering 0px corners rather than the intended 4px control radius when this observation was made. This row was judged under a broken condition and requires re-verification after gap closure. |
...
- [ ] `nyquist_compliant: true` set in frontmatter — **not set.** The browser checkpoint returned PARTIAL with a shipped CSS defect; see Gap-Closure Record below. `/gsd-plan-phase 19 --gaps` must plan and execute gap closure, and rows 1, 3, 6 and 12 must be re-verified in a real browser, before this can flip to `true`.

**Approval:** PARTIAL 2026-08-12 — 8 of 13 Manual-Only Verifications rows confirmed clean (rows 2, 4 hover-half, 5, 7, 8, 9, 10, 11, 13 — note row 4's click-half and row 5's Calendar/Banister sub-checks are separately annotated, not blanket passes). Rows 1, 3 and 12 were each judged under a broken condition (dead `--radius-control` token) and cannot be signed off — they are BLOCKED BY DEFECT, not passes. Row 6 (focus ring) FAILS outright on the detail view's segmented control. `nyquist_compliant: false`. `requirements-completed` for this checkpoint lists only `UI-03`, `ACT-01` — see below.

**requirements-completed (this checkpoint):**

- **CONFIRMED:** `UI-03` (row 7, row 13 — both pass; UI-03 depends only on rows 7 and 13 per the row→requirement map), `ACT-01` (rows 8-9 — both pass)
- **NOT COMPLETED:**
  - `UI-01` — BLOCKED. Depends on rows 1-2. Row 1 is blocked by the dead `--radius-control` defect (control-baseline inputs judged with 0px corners instead of the intended 4px). Row 2 passed on its own terms but is recorded alongside a separate non-Phase-19 cross-browser gap (Calendar month picker).
  - `UI-02` — BLOCKED. Depends on rows 3-6 and 10-13. Row 3 is blocked by the same dead-token defect (all 12 button treatments judged with 0px corners). Row 6 (focus ring) fails outright — the detail segmented control's ring is occluded. Row 12 is blocked by the same dead-token defect, invalidated as described in its Observation cell. Rows 4 (hover-half), 5, 10, 11 and 13 pass on their own terms but do not clear UI-02 while rows 3, 6 and 12 remain open.
...
2. This phase authorized no new test dependency (no jsdom, no happy-dom, no `@testing-library/*`, no puppeteer, no playwright); none was installed.
...
`nyquist_compliant` remains `false` and the phase gate does **not** close. The next step is `/gsd-plan-phase 19 --gaps` to plan and execute closure of GAP 1, GAP 2 and GAP 3 above, followed by re-verification in a real browser of rows 1, 3, 6 and 12 specifically (the rows blocked or failed by this checkpoint).
