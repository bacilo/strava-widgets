...
class StreakWidgetElement extends WidgetBase {
    ...
    render(data) {
        ...
        const currentStreakDetail = data.withinCurrentStreak && data.currentStreakStart
            ...
            : '';
        streakGrid.appendChild(this.createStreakItem(currentStreakValue, 'Current Streak', currentStreakDetail));
        // Longest streak
        const longestStreakDetail = data.longestStreakStart && data.longestStreakEnd
            ? `${this.formatDate(data.longestStreakStart)} - ${this.formatDate(data.longestStreakEnd)}`
            : '';
        streakGrid.appendChild(this.createStreakItem(`${data.longestStreak}`, 'Longest Streak', longestStreakDetail));
        ...
        const consistencyValue = data.weeklyConsistency.currentStreak > 0
            ...
            : '—';
        streakGrid.appendChild(this.createStreakItem(consistencyValue, 'Weekly Consistency', `${data.weeklyConsistency.minRunsPerWeek}+ runs/week`));
        // Longest weekly consistency
        streakGrid.appendChild(this.createStreakItem(`${data.weeklyConsistency.longestStreak}`, 'Longest Consistency', `${data.weeklyConsistency.totalConsistentWeeks} total weeks`));
     ...
     */
    createStreakItem(value, label, detail = '') {
...
const StreakWidgetInit = {
    async init(containerId, config) {
        ...
        const widget = document.createElement('strava-streak-widget');
...
window.StreakWidget = StreakWidgetInit;
