...
class StatsCardWidget extends WidgetBase {
    ...
    async fetchDataAndRender() {
        try {
            ...
            const allTimeData = await this.fetchData(primaryUrl);
            ...
            if (secondaryUrl && showYoY) {
                try {
                    this.yearOverYearData = await this.fetchData(secondaryUrl);
                }
                catch (error) {
    ...
    render(data) {
        ...
        // Total distance
        const distanceItem = this.createStatItem(this.formatNumber(allTimeData.totalKm, 0), 'Total KM');
        statsGrid.appendChild(distanceItem);
        // Total runs
        const runsItem = this.createStatItem(this.formatNumber(allTimeData.totalRuns, 0), 'Total Runs');
        statsGrid.appendChild(runsItem);
        // Total hours
        const hoursItem = this.createStatItem(this.formatNumber(allTimeData.totalHours, 0), 'Total Hours');
        statsGrid.appendChild(hoursItem);
        // Average pace
        const paceItem = this.createStatItem(this.formatPace(allTimeData.avgPaceMinPerKm), 'Avg Pace');
     ...
     */
    createYearOverYearSection(yoyData) {
        ...
        // Previous year
        const prevItem = this.createYoyItem(previousYear, this.formatNumber(previousYearTotal, 0) + ' km', null);
        grid.appendChild(prevItem);
        // Current year
        const currItem = this.createYoyItem(currentYear, this.formatNumber(currentYearTotal, 0) + ' km', delta);
     ...
     */
    createYoyItem(year, value, delta) {
        const item = document.createElement('div');
...
const StatsCard = {
    async init(containerId, config) {
        ...
        const element = document.createElement('strava-stats-card');
