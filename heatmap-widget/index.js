...
class HeatmapWidgetElement extends WidgetBase {
     ...
     */
    renderHeatmapFromRoutes(routes) {
        if (!this.map)
            return;
    ...
    renderControls() {
      ...
      <div class="section">
        ...
        <div class="color-buttons">
          ${Object.keys(COLOR_SCHEMES).map(scheme => `<button data-scheme="${scheme}" class="${scheme === this.currentScheme ? 'active' : ''}">${scheme}</button>`).join('')}
        </div>
      </div>
    ...
    attachControlListeners(controls) {
        ...
        controls.querySelector('#apply-custom')?.addEventListener('click', () => {
            const start = controls.querySelector('#start-date').value;
            const end = controls.querySelector('#end-date').value || new Date().toISOString().split('T')[0];
            if (start) {
     ...
     */
    applyCustomDateFilter(startStr, endStr) {
     ...
     */
    filterAndRenderByDateRange(start, end) {
