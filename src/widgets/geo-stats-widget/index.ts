...
class GeoStatsWidgetElement extends WidgetBase {
  ...
  private async fetchAllDataAndRender(): Promise<void> {
    try {
      ...
      const [countries, cities, metadata] = await Promise.all([
        this.fetchData<CountryStats[]>(primaryUrl),
        secondaryUrl
          ? this.fetchData<CityStats[]>(secondaryUrl)
          : Promise.resolve([]),
        metadataUrl
          ? this.fetchData<GeoMetadata>(metadataUrl)
   ...
   */
  private createCountriesSection(countries: CountryStats[], showExport: boolean): HTMLElement {
   ...
   */
  private createCitiesSection(cities: CityStats[], showExport: boolean): HTMLElement {
    const section = document.createElement('div');
...
const GeoStatsWidget = {
  async init(containerId: string, config: any): Promise<void> {
    ...
    const widget = document.createElement('strava-geo-stats') as GeoStatsWidgetElement;
