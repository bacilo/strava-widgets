...
export const PINNED_FIXTURES: readonly PinnedFixture[] = [
  ...
  },
  {
    ...
    deviceFamily: 'suunto-9',
    streamSource: 'fit',
    why: "the pinned PACE-04 exemplar; span 3394s vs metadata elapsed_time 3393, distance 10130m, 28.7% zero-advance; also Phase 30's sub-ground exemplar (minimum altitude -282 m, well below the -50 m threshold), a loop (startEndDistM 0) whose own drift (-5.6 m) and worst vertical rate (3.3 m/s) both stay clear — confirming the sub-ground fixture does not also trip the other two elevation modes",
    expected: {
      spanSec: 3394,
      elapsedTimeSec: 3393,
      distanceM: 10130,
      zeroAdvanceFraction: 0.287,
      minAltM: -282,
      driftDeltaM: -5.6,
      startEndDistM: 0,
      worstRateMps: 3.3,
  ...
  {
    name: 'closure-drift-exemplar',
    activityId: '4745489664',
    deviceFamily: 'suunto-9',
    streamSource: 'fit',
    why: 'the archive\'s worst loop-gated closure-drift real: drifts -197.6 m over a loop whose start and end are the same point (startEndDistM 0), while its own minimum altitude (4.2 m) and worst vertical rate (2.3 m/s) both stay clear of their thresholds — the archive\'s drift-isolated real exemplar',
    expected: { driftDeltaM: -197.6, startEndDistM: 0, minAltM: 4.2 },
  },
  {
    name: 'vertical-rate-exemplar',
    activityId: '3149636661',
    deviceFamily: 'garmin-vivoactive-4',
    streamSource: 'gpx',
    why: 'the archive\'s worst vertical-rate real (80.4 m/s), but this activity ALSO drifts (loop-gated, -172.6 m over a startEndDistM-0 loop) — it fires BOTH modes, so it is a reported real exemplar and must NEVER be used as a single-mode isolation fixture; that isolation proof belongs to plan 30-01\'s three synthetics. driftDeltaM is carried in `expected` deliberately, making the dual-mode fact machine-checked rather than a comment',
    expected: { worstRateMps: 80.4, driftDeltaM: -172.6, startEndDistM: 0, minAltM: 19.8 },
  },
] as const;
